...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        #hs,hidden=self.rnn(embeddings)
        hidden,hs=self.rnn(embeddings)
        #return hs
        return hidden
...
class seq2seqAtt(nn.Module):
    ...
    def forward(self, target_h, source_hs):
        ...
        concat_output=self.ff_concat(torch.concat((target_h_rep,source_hs),dim=-1))
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
      apply tanh, and make the prediction
        # prediction should be of shape (1, batch, vocab), h and tilde_h of shape (1, batch, feat)
        embeddings=self.embedding(input)
        rnn,target_h=self.rnn(embeddings,h)
        concat=torch.concat((source_context,rnn),dim=-1)
        #concat=torch.concat((rnn,source_context),dim=-1)

        concat=self.ff_concat(concat)
        tilde_h=torch.tanh(concat)
        #h=tilde_h
        prediction=self.predict(tilde_h)
        #prediction=torch.softmax(prediction)
        return prediction, target_h

x=torch.tensor([[ 90,   5,  20,  23,   5,  14,   8,  14, 429,  25,  37, 140,   5,  20,
           5,   8,  38,  33,   5,  44,  37,   5,   5,  28, 205,   5,  13,   5,
           5,  13,  49,  16,  15,   8,   5,  53,  44,   5,   6,  29,  14,  33,
          37,  70,  31,  16,   5,   5,  14,  44,  71,  16, 272,  44,  28,   5,
          25,   5,   6,   5,  20,   6,   5,  48]])

x = torch.LongTensor([1]).repeat(64).unsqueeze(0) # init (1, batch)
print(x)
...
class seq2seqModel(nn.Module):
    ...
    def forward(self, input, max_size, is_prod):
        ...
        while True:
            ...
            target_input = prediction.argmax(dim=-1)# the predicted word
...
!wget -c "/content" -O "data.zip"
!wget -c "/content" -O "pretrained_moodle.pt"
!unzip data.zip

path_to_data = '/content/data/'
path_to_save_models = '/content/'
